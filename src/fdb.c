/*
 * Falcon device binding: proof that a Falcon-512 signature was made by the key
 * committed in a credential. See fdb.h for the protocol overview and API.
 *
 ...
 *   (s2*h)_iso[m] = sum_{l+k=m} hiso[k]*s2iso[l]
 *                 + sum_{l+k=m+8} Y*hiso[k]*s2iso[l]
 * i.e. the R2 coefficient is 1 on the plain terms and Y on the wrapped ones,
 * where Y = X^8 is represented in the deg-64 ring by the degree-1 monomial X
 * (validated against M_h*s2iso).  k is the mod-p quotient (route b).
 *
...
#include "fdb.h"
#include "fdb-params.h" /* defines lnp_tbox_params_t fdb_param */
...
#define NEQ FDB_NEQ
#define NLIN FDB_NLIN
#define NEQTOT FDB_NEQTOT
#define RC_BLOCKS FDB_RC_BLOCKS
#define ANON_P 12289
#define ANON_DEG 64
 ...
 * the l2 budget of the whole vector (k, FSCALE*h), sizing stdev4); FSCALE is
 * the only one of the two this file needs -- Bprime's effects are carried by
 * the generated params -- and it must stay in sync with the value there. */
               ...
               unsigned int Z, unsigned int l)
{
  ...
  /* like src/lnp.c's _scatter_smat: reset the target, so a second scatter into
   * the same R2 (e.g. re-running fdb_public_setup on one state) replaces the
   * entries instead of appending duplicates -- which would silently double
              ...
              unsigned int Z)
{
  ...
  spolyvec_set_empty (r1); /* see _scatter_smat: reset, do not append */
  ...
  for (i = 0; i < NEQ; i++)
    {
       ...
       * hiso[k]=slot 64+2k; coeff 1 when l+k=i, coeff Y on the negacyclic wrap
       * l+k=i+8, written as the degree-1 monomial X (see the file header). */
  ...
  for (i = 0; i < 8; i++)
    {
       ...
       * the credential commitment binding (see fdb.h and fdb-params.sage). */
...
fdb_public_setup (polymat_t A1, polymat_t A2prime, polymat_t Bmat,
                  ...
                  unsigned int m1, unsigned int Z, unsigned int l,
                  unsigned int n, unsigned int n_, const uint8_t seed[32])
{
  uint8_t seed_[32];

  memcpy (seed_, seed, 32); /* the expansions take a non-const seed */
  abdlop_keygen (A1, A2prime, Bmat, seed_, tbox);
  /* domains 100/101 separate AR/AM from abdlop_keygen's seed expansions */
  polymat_urandom (ARmat, Rq->q, Rq->log2q, seed_, 100);
  polymat_urandom (AMmat, Rq->q, Rq->log2q, seed_, 101);
              ...
              lnp_tbox_params_srcptr params)
{
   ...
   * public and derived by the verifier itself.  Sending them would let a
   * dishonest signer choose the equations' constant terms and break the
   * binding to c and t_h (see falcon-devicebind-soundness-issue.md). */
...
/* ---- Fiat-Shamir transcript prefix, shared by both roles.
 *
 * lnp_tbox_prove/verify take the transcript hash as an INPUT and never absorb
 * tA1 or the statement themselves (see src/lnp-tbox.c: "hash hash of tA1,
 * tB"), so the caller must do it -- exactly as the production caller
 * lin-proofs.c does (__lnp_hash_pp_and_statement, then abdlop_hashcomm).
 * Without this the challenge c is independent of the Ajtai commitment tA1 and
 * of the statement, and a prover could pick tA1 after learning c.
 *
 * hash = H(seed || salt || msglen || msg || enc(t_h)), i.e. everything the
 * public data is derived from: the setup seed fixes A1/A2prime/Bmat, AR/AM and
 * the R2/r1/Es/Dm structure; (salt, msg) fix the challenge c and hence
 * r0[0..NEQ) = -c; t_h fixes r0[NEQ..NEQTOT).  The verifier re-encodes its own
 * DECODED t_h, so the transcript binds the credential's canonical value rather
 * than the bytes a peer happened to send.  The call sites then mix in tA1/tB
 * via abdlop_hashcomm. */
static void
fdb_hash_statement (uint8_t hash[32], const uint8_t seed[32], polyvec_t th,
                    polyring_srcptr Rq, const uint8_t *msg, size_t msglen,
                    const uint8_t salt[FDB_SALT_BYTES])
{
  uint8_t credbuf[FDB_CRED_MAXLEN];
  uint8_t msglenbuf[8];
  ...
  /* fdb_enccred mutates its argument into the redp representation: encode a
   * copy so the caller's th stays centered. */
  polyvec_alloc (thc, Rq, NLIN);
  polyvec_set (thc, th);
  fdb_enccred (credbuf, &credlen, thc, Rq);
  polyvec_free (thc);

  /* absorb msglen explicitly: the salt is fixed-size, but msg is not, so
   * without a length the (salt, msg) pair would not be uniquely parseable. */
  for (i = 0; i < 8; i++)
    msglenbuf[i] = (uint8_t)(((uint64_t)msglen >> (8 * i)) & 0xff);

  shake128_init (hstate);
  shake128_absorb (hstate, seed, 32);
  shake128_absorb (hstate, salt, FDB_SALT_BYTES);
  shake128_absorb (hstate, msglenbuf, sizeof (msglenbuf));
  shake128_absorb (hstate, msg, msglen);
  shake128_absorb (hstate, credbuf, credlen);
...
/* ==== signer =============================================================
 * Holds the Falcon-512 keypair in plain, signs whatever message it is
 * given, and builds the proof. */

void
fdb_keygen (uint8_t sk[FDB_PRIVKEYLEN], uint8_t pk[FDB_PUBKEYLEN])
{
  falcon_keygen (sk, pk);
}
 ...
 * close. */
static void
compute_hiso (fdb_signer_state_t s)
{
...
static void
build_isoring_reps (fdb_signer_state_t s)
{
...
int
fdb_signer_issue (fdb_signer_state_t s, uint8_t *cred, size_t *credlen)
{
  polymat_t ARc, AMc;
  polyvec_t hq, thc;
  uint8_t rcseed[32];
  ...
  /* serialize a COPY: fdb_enccred mutates its argument into the positive
   * (redp) representation, and s->th stays centered for later use. */
  polyvec_alloc (thc, s->Rq, NLIN);
  polyvec_set (thc, s->th);
  fdb_enccred (cred, credlen, thc, s->Rq);
  polyvec_free (thc);

  polymat_free (ARc);
  polymat_free (AMc);
  polyvec_free (hq);
  explicit_bzero (rcseed, sizeof (rcseed)); /* regenerates the secret rc */
  s->issued = 1;
  return 1;
}
...
static void
build_witness (fdb_signer_state_t s)
{
  unsigned int blk;
...
static int
compute_quotient (fdb_signer_state_t s)
{
 ...
 *
 * With the quadratic structure constants of build_equation_structure the
 * equation evaluates, at the honest witness, to
 ...
 * verifier, never transmitted.  This is what binds the proof to the
 * verifier's message; see falcon-devicebind-soundness-issue.md. */
static void
...
static void
setup_l2_slack (fdb_signer_state_t s, lnp_tbox_params_srcptr params)
{
...
void
fdb_signer_init (fdb_signer_state_t s, const uint8_t seed[32],
                 const uint8_t sk[FDB_PRIVKEYLEN],
                 const uint8_t pk[FDB_PUBKEYLEN])
{
  lnp_tbox_params_srcptr params = fdb_param;
  polyring_srcptr Rq;
  ...
  memcpy (s->seed, seed, 32);
  memcpy (s->sk, sk, FDB_PRIVKEYLEN);
  memcpy (s->pk, pk, FDB_PUBKEYLEN);
  s->issued = 0; /* rc/th are unset until fdb_signer_issue runs */
...
void
fdb_signer_clear (fdb_signer_state_t s)
{
  unsigned int i;

  for (i = 0; i < NEQ; i++)
    spolymat_free (s->R2ii[i]);
  for (i = 0; i < NEQTOT; i++)
    spolyvec_free (s->r1ii[i]);

  /* wipe the secret witness material before freeing it: the isoring key
   * representation, the commitment randomness rc and s2, and the witness
   * vectors s1/mvec (which hold hiso and the quotient k). */
  polymat_set_zero (s->Mh);
  polyvec_set_zero (s->hiso);
  polyvec_set_zero (s->ciso);
  polyvec_set_zero (s->s1iso);
  polyvec_set_zero (s->s2iso);
  polyvec_set_zero (s->rc);
  polyvec_set_zero (s->s1);
  polyvec_set_zero (s->s2);
  polyvec_set_zero (s->mvec);
  poly_set_zero (s->hf);
  poly_set_zero (s->s1f);
  poly_set_zero (s->s2f);
  ...
  polymat_free (s->Dm);
  polymat_free (s->A1);
  polymat_free (s->A2prime);
  polymat_free (s->Bmat);
  polymat_free (s->ARmat);
  polymat_free (s->AMmat);
  polymat_free (s->Mh);

  polyvec_free (s->hiso);
  polyvec_free (s->ciso);
  polyvec_free (s->s1iso);
  polyvec_free (s->s2iso);
  polyvec_free (s->rc);
  polyvec_free (s->th);
  polyvec_free (s->s1);
  polyvec_free (s->s2);
  ...
  explicit_bzero (s->sk, sizeof (s->sk));
  explicit_bzero (s->hc16, sizeof (s->hc16));
  explicit_bzero (s->s1c16, sizeof (s->s1c16));
  explicit_bzero (s->s2c16, sizeof (s->s2c16));
}

int
fdb_signer_prove (fdb_signer_state_t s, uint8_t *proof, size_t *prooflen,
                  uint8_t salt[FDB_SALT_BYTES], const uint8_t *msg,
                  size_t msglen)
{
  lnp_tbox_params_srcptr params = fdb_param;
  int16_t cc16[512];
  ...
  if (!s->issued)
    return 0; /* no credential issued yet: rc (and t_h) are unset */

  /* blackbox signing: message in -> (salt, s1, s2) out; the key never leaves.
   * (s1, s2) short with s1 + s2*h = HashToPoint(salt||msg) (mod p). */
  falcon_sign_message (salt, s->s1c16, s->s2c16, msg, msglen, s->sk);

  /* recompute the challenge c that was hashed to (public: salt + msg) */
  ...
  if (!compute_quotient (s))
    return 0; /* s1+s2h-c not 0 mod p: relation/iso mismatch */
  ...
  /* transcript: public parameters + statement, then the commitment */
  fdb_hash_statement (s->hashp, s->seed, s->th, s->Rq, msg, msglen, salt);
  abdlop_hashcomm (s->hashp, s->tA1, s->tB, s->tbox);
  ...
  explicit_bzero (secseed, sizeof (secseed));
  return 1;
}
...
void
fdb_verifier_init (fdb_verifier_state_t v, const uint8_t seed[32])
{
  lnp_tbox_params_srcptr params = fdb_param;
  polyring_srcptr Rq;
  unsigned int i;
  ...
  memcpy (v->seed, seed, 32);
  /* the credential encoding is fixed-width (NLIN*d coefficients packed in
   * log2q bits each), so its length is a constant of the parameters: record it
   * once here and reject any other length before decoding (fdb_deccred, like
   * the rest of lazer's coder, has no input bound of its own). */
  {
    uint8_t credbuf[FDB_CRED_MAXLEN];
    polyvec_t thz;
    polyvec_alloc (thz, Rq, NLIN);
    polyvec_set_zero (thz);
    fdb_enccred (credbuf, &v->credlen, thz, Rq);
    polyvec_free (thz);
  }
  v->has_cred = 0; /* th/r0[NEQ..NEQTOT) are unset until a credential is set */

  for (i = 0; i < NEQTOT; i++)
    {
      poly_alloc (v->r0ii[i], Rq);
      /* poly_alloc does not initialize: zero, so a stray verify before
       * fdb_verifier_set_credential cannot read uninitialized limbs */
      poly_set_zero (v->r0ii[i]);
      v->r0[i] = v->r0ii[i];
    }
  polyvec_set_zero (v->th);
...
void
fdb_verifier_clear (fdb_verifier_state_t v)
{
  unsigned int i;

  for (i = 0; i < NEQ; i++)
    spolymat_free (v->R2ii[i]);
  for (i = 0; i < NEQTOT; i++)
    {
      spolyvec_free (v->r1ii[i]);
      poly_free (v->r0ii[i]);
    }
  ...
  polymat_free (v->Dm);
  polymat_free (v->A1);
  polymat_free (v->A2prime);
  polymat_free (v->Bmat);
  polymat_free (v->ARmat);
  polymat_free (v->AMmat);
...
static void
fdb_verifier_refresh_credential_r0 (fdb_verifier_state_t v)
{
  unsigned int i;
  for (i = 0; i < NLIN; i++)
    {
...
int
fdb_verifier_set_credential (fdb_verifier_state_t v, const uint8_t *cred,
                             size_t credlen)
{
  size_t declen;

  /* check the length BEFORE decoding: a short buffer would otherwise be read
   * past its end (the decoder consumes the full fixed width regardless). */
  if (credlen != v->credlen)
    return 0;
...
int
fdb_verifier_vrfy (fdb_verifier_state_t v, const uint8_t *proof,
                   ...
                   const uint8_t salt[FDB_SALT_BYTES])
{
  ...
  if (!v->has_cred)
    return 0; /* nothing to verify against: no credential stored */

  /* The proof encoding is variable-length (ghint / discrete-Gaussian coders),
   * so there is no exact length to check up front, and lazer's decoder takes
   * no input bound -- as in lin-proofs.c, a truncated buffer is only caught by
   * the declen comparison below, after decoding.  Reject what can be rejected
   * up front; do NOT compare against params->prooflen, which is codegen's
   * EXPECTED size, not an upper bound (the ghint/Gaussian coders vary by a few
   * bytes per proof, so an honest proof may exceed it). */
  if (prooflen > FDB_PROOF_MAXLEN)
    return 0;
  ...
  /* transcript: public parameters + statement, then the commitment (same
   * order and inputs as the signer -- see fdb_hash_statement) */
  fdb_hash_statement (v->hashv, v->seed, v->th, v->Rq, msg, msglen, salt);
  abdlop_hashcomm (v->hashv, v->tA1, v->tB, v->tbox);
